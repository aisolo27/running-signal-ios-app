...
- Normal workout detail now has an internal gate for only the supported narrow class: `Warmup(2 km) > one Work step > Cooldown(Open)`. When the gate passes, the existing Apple Fitness Intervals section can show reconstructed Warmup/Work/Cooldown rows; repeat blocks, Open/Extra tails, missing evidence, and unsupported shapes keep the existing placeholder behavior.
...
- Next implementation should physically verify the normal workout detail Apple Fitness Intervals section on March 5, 2026 and April 24, 2026, then confirm April 22, 2026 and May 1, 2026 still keep the existing blocked/placeholder behavior.
