...
- Raw HealthKit Debug and parity packet exports now also include a debug-only `HKWorkout.workoutActivities` inventory when public `HKWorkoutActivity` rows are available. This is a new source-investigation lead only; active physical-device fixture exports need regeneration before judging whether activity boundaries explain FIT/Apple row timing.
...
- Regenerate Raw HealthKit Debug markdown and parity packet JSON for the active fixture set after the `HKWorkoutActivity` export enhancement before updating the FIT/Apple boundary-source conclusion.
