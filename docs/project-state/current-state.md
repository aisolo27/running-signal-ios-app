...
- Raw HealthKit Debug and parity packet exports now also include a debug-only `HKWorkout.workoutActivities` inventory when public `HKWorkoutActivity` rows are available, plus an `hkworkoutactivity_boundary` candidate reconstruction shown side-by-side with current reconstructed intervals. Latest regenerated physical-device exports are archived for the full active fixture set and confirm the on-device export fields `activityBoundaryCandidateSummary` and `activityBoundaryCandidateIntervals` are present. `HKWorkoutActivity` boundaries remain the strongest public-API lead: the debug scorer improves the May 26, June 1, and June 12 drift cases, but June 4 regresses from preferred pass to temporary pass and June 3 has three special-fixture regressions. This remains diagnostics/export-only; production reconstruction is unchanged.
- Raw HealthKit Debug now also has a diagnostics-only monthly evidence refresh plus export workflow for a selected month. `Refresh Month Evidence` invalidates each loaded workout's evidence cache for that month, reruns HealthKit/WorkoutKit evidence queries one workout at a time, records per-workout refresh status, and then the monthly JSON/markdown exports show fresh-query versus cached/missing evidence state. It is debug/research-only and does not change normal workout UI, interval reconstruction, or boundary selection.
...
- Physical-device parity packet collection has succeeded for the active fixture set; candidate boundary scoring and the activity-boundary export prototype remain debug-only and no production boundary strategy is approved. Activity-boundary scoring needs more guard coverage before any production experiment.
...
- Next parity step: on the physical iPhone, run `Refresh Month Evidence` before exporting each monthly diagnostics bundle, then collect more Work + Open guard fixtures now that the active fixture Raw HealthKit Debug markdown and parity packet JSON exports with `activityBoundaryCandidateSummary` and `activityBoundaryCandidateIntervals` are archived.
