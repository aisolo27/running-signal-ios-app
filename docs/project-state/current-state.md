...
- Normal workout detail now has internal gates for two narrow classes only: `Warmup(2 km) > one Work step > Cooldown(Open)` and `Warmup(2 km) > one Work step > fixed Cooldown > inferred Open / Extra tail` when planned rows map cleanly to HealthKit activity rows and the only extra row is the final tail from last activity end to workout end. Physical iPhone screenshots from 2026-06-14 confirmed March 5, 2026 and April 24, 2026 show reconstructed Warmup/Work/Cooldown rows, while April 22, 2026 and May 1, 2026 keep the blocked placeholder behavior. Repeat blocks, recovery-containing tail cases, missing evidence, and unsupported shapes stay blocked.
...
- Next implementation should keep the narrow normal-detail gates in place and avoid broad interval promotion until repeat-block, recovery-containing Open/Extra-tail, and repeat-tail rules are explicitly approved.
