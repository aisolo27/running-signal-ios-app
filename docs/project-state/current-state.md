...
- Raw HealthKit Debug and parity packet exports now also include a debug-only `HKWorkout.workoutActivities` inventory when public `HKWorkoutActivity` rows are available, plus an `hkworkoutactivity_boundary` candidate reconstruction shown side-by-side with current reconstructed intervals. Regenerated physical-device exports are archived for the previous activity inventory pass, but need a new export pass for the candidate rows. `HKWorkoutActivity` boundaries are now the strongest public-API lead: the debug scorer improves the May 26, June 1, and June 12 drift cases, but June 4 regresses from preferred pass to temporary pass and June 3 has three special-fixture regressions. This remains diagnostics/export-only; production reconstruction is unchanged.
...
- Next parity step: regenerate physical-device Raw HealthKit Debug markdown and parity packet JSON for the active fixtures so `activityBoundaryCandidateSummary` and `activityBoundaryCandidateIntervals` can be archived, then collect more Work + Open guard fixtures.
