...
- Next custom-workout work should regenerate selected-workout parity packets from the app for May 1 and representative repeat/Open-tail cases, then compare the new `customWorkoutCandidateRuleRows` against the docs/debug scorecard. Normal workout detail UI and production interval behavior remain blocked unless a later task explicitly approves them.
