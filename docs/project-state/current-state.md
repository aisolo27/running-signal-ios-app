...
- Use `docs/validation/apple-fitness-interval-parity-dataset/paused-repeat-open-extra-debug-prototype-plan-2026-06-24.md` for the paused repeat fixed-tail `Open / Extra` debug/export prototype; Raw HealthKit Debug/parity output and the in-app Parity Lab status expose row/tail/pause/fallback diagnostics and standardized fallback labels. June 26 proof in `docs/validation/apple-fitness-interval-parity-dataset/physical-iphone-paused-repeat-fixed-tail-open-extra-proof-2026-06-26/` satisfies rung 2 `Evidence Available` for the exact paired-pause fixed-tail row, but does not promote normal detail or broaden Gate B. For fresh proof folders exported by builds with readable fallback labels, run `docs/validation/apple-fitness-interval-parity-dataset/summarize_parity_proof_folder.py <proof-folder>` and `docs/validation/apple-fitness-interval-parity-dataset/validate_parity_export_consistency.py --require-readable-fallback-labels <proof-folder>` before archiving or promotion review.
