...
- Raw HealthKit Debug now also has a diagnostics-only monthly export for the selected workout's month. It shares one monthly JSON bundle with per-workout parity-packet-style records plus a compact markdown summary, classifies loaded workouts for research triage, and keeps FIT/Apple Fitness/manual references out of runtime logic. It is debug/research-only and does not change normal workout UI, interval reconstruction, or boundary selection.
...
- Next parity step: use the monthly diagnostics export to collect broader month-scale evidence, then collect more Work + Open guard fixtures now that the active fixture Raw HealthKit Debug markdown and parity packet JSON exports with `activityBoundaryCandidateSummary` and `activityBoundaryCandidateIntervals` are archived.
