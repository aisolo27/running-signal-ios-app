...
- Normal workout detail now has an internal gate for only the supported narrow class: `Warmup(2 km) > one Work step > Cooldown(Open)`. Physical iPhone screenshots from 2026-06-14 confirmed March 5, 2026 and April 24, 2026 show reconstructed Warmup/Work/Cooldown rows, while April 22, 2026 and May 1, 2026 keep the blocked placeholder behavior. Repeat blocks, Open/Extra tails, missing evidence, and unsupported shapes stay blocked.
...
- Next implementation should keep the narrow normal-detail gate in place and avoid broad interval promotion until repeat-block and Open/Extra-tail rules are explicitly approved.
