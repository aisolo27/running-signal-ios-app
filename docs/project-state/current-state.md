...
- Next screenshot-backed validation work can move from evidence collection to docs/debug rule scoring: active/timer row duration, paired pause overlap, and fixed-step exhaustion before `Open / Extra` tail classification. May 1 no longer blocks on paired pause-debug evidence.
