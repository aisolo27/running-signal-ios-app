...
- Next custom-workout work should use the fresh Parity Lab export review as supporting evidence, then decide whether more balanced evidence is needed before any explicit debug-only Phase 3 prototype discussion. Normal workout detail UI and production interval behavior remain blocked unless a later task explicitly approves them.
