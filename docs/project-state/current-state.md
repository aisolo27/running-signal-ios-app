...
- Next implementation should first spot-check the existing Parity Lab structured status/fallback output on physical iPhone evidence. If it looks correct for the narrow `Warmup(2 km) > one Work step > Cooldown(Open)` class, the next ticket can be an internal-gated normal workout detail integration for that narrow class only.
