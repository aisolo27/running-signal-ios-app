...
- Normal workout detail now has internal gates for five narrow classes only: stopped-early `one fixed-distance Work step` mapped to one complete partial HealthKit activity row, `Warmup(2 km) > one Work step > Cooldown(Open)`, `Warmup(2 km) > one Work step > fixed Cooldown > inferred Open / Extra tail`, clean no-pause repeat blocks shaped as `Warmup(2 km) > repeated Work/Recovery rows > Cooldown(Open)`, and clean no-pause repeat blocks shaped as `Warmup(2 km) > repeated Work/Recovery rows > fixed Cooldown > inferred Open / Extra tail` when planned rows map one-to-one to complete contiguous HealthKit activity rows and the only extra row is the final tail. Physical iPhone screenshots from 2026-06-14 confirmed March 5, 2026 and April 24, 2026 show reconstructed Warmup/Work/Cooldown rows, and `docs/validation/apple-fitness-interval-parity-dataset/physical-iphone-repeat-block-proof-2026-06-14/` confirms May 20 and June 3 render expanded repeat rows with final `Cooldown` and no `Open / Extra` tail, while June 10 renders expanded repeat rows, fixed `Cooldown`, and final `Open / Extra`. Normal-detail gates require paired pauses to be absent for promoted time-goal rows and now use mapped HealthKit activity boundaries for displayed approved intervals instead of re-blocking clean activity-backed rows when the older distance/time reconstruction drifts. Blocked normal-detail cases append a short failed-gate reason, while still keeping unsupported rows hidden. April 22, 2026 and May 1, 2026 keep the blocked placeholder behavior. Paused repeat blocks, recovery-containing tail cases, ambiguous repeat tails, missing evidence, and unsupported shapes stay blocked.
...
- Gate A: FIT supports a narrow debug-only `HKWorkoutActivity` prototype for simple fixed-distance Work + Open only. Raw HealthKit Debug and parity packet exports now opt into that prototype for the exact one fixed-distance Work step + one complete HealthKit activity row + positive Open/Extra tail shape. Normal workout detail remains blocked for simple Work/Open.
...
- Paused repeat-block timer semantics are now defined for docs/debug work in `docs/validation/apple-fitness-interval-parity-dataset/paused-repeat-block-timer-rule-2026-06-15.md`: keep elapsed HealthKit activity windows, compute active/timer duration by subtracting paired HealthKit pause overlap, and keep paused repeat blocks blocked from normal workout detail until a later debug prototype proves row mapping and guard behavior.
- Recovery-containing Open/Extra tail semantics are now defined for docs/debug work in `docs/validation/apple-fitness-interval-parity-dataset/recovery-containing-open-tail-rule-2026-06-15.md`: preserve planned Recovery rows, map all fixed planned rows to complete HealthKit activity rows, and infer Open/Extra only after fixed planned rows are exhausted. Normal workout detail remains blocked until a later debug prototype proves separator and guard behavior.
- Ambiguous repeat-tail semantics are now defined for docs/debug work in `docs/validation/apple-fitness-interval-parity-dataset/ambiguous-repeat-tail-rule-2026-06-15.md`: expand repeat blocks, map every planned row, keep open cooldowns labeled Cooldown, and infer Open/Extra only after a resolved fixed final row. Normal workout detail remains blocked until a later debug prototype proves repeat-tail guards.
- Simple fixed-distance Work/Open Gate A prototype boundaries are defined in `docs/validation/apple-fitness-interval-parity-dataset/simple-work-open-prototype-decision-2026-06-15.md` and implemented in debug structured comparison only: exactly one fixed-distance Work step, one complete HealthKit activity row, and a positive Open/Extra tail. Normal workout detail remains blocked.
...
- Continue Gate B work by reviewing row-level outliers and defining repeat-block plus Open/Extra tail rules before changing structured interval or warmup/work/cooldown behavior. Gate A simple Work/Open now has debug-only Parity Lab/export support, but remains blocked from normal workout detail.
