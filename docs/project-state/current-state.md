...
- Raw HealthKit Debug now renders a debug-only "Parity Lab Candidate Rows" section in-app. It mirrors the scorer shape by showing candidate row count, Open-tail count, paired pause count/total, elapsed duration, pause overlap, active duration, distance, and row mapping status. It remains debug-only and does not replace normal workout detail intervals.
...
- Next custom-workout work can either commit/push the completed debug evidence and in-app Parity Lab section, or move into an explicitly scoped next phase: broader physical-phone spot checks or a production-readiness plan. Normal workout detail UI and production interval behavior remain blocked unless a later task explicitly approves them.
