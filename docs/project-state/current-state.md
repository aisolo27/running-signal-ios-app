...
- A HealthFit FIT comparison pilot is documented and archived for the same packet-backed running fixtures. FIT exports remain research-only and are not a production import path, HealthFit dependency, or replacement for Apple Fitness visual parity. The pilot found FIT session totals match RunSignal totals, while FIT lap rows and inferred Open tails often align more closely with Apple Fitness/manual row timing.
...
- Use `fit-comparison-summary.md` for the completed docs/debug FIT cross-check. FIT evidence can inform investigation, but it does not approve boundary logic or production app changes.
