...
- Next implementation should first spot-check regenerated Parity Lab structured status/fallback exports for March 5, 2026 and April 24, 2026 on physical iPhone evidence. If both now report the narrow `Warmup(2 km) > one Work step > Cooldown(Open)` class as supported and blocked subclasses still report exact fallback reasons, the next ticket can be an internal-gated normal workout detail integration for that narrow class only.
