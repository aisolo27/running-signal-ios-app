...
- Normal workout detail now has internal gates for three narrow classes only: `Warmup(2 km) > one Work step > Cooldown(Open)`, `Warmup(2 km) > one Work step > fixed Cooldown > inferred Open / Extra tail`, and clean no-pause repeat blocks shaped as `Warmup(2 km) > repeated Work/Recovery rows > Cooldown(Open)` when planned rows map one-to-one to HealthKit activity rows. Physical iPhone screenshots from 2026-06-14 confirmed March 5, 2026 and April 24, 2026 show reconstructed Warmup/Work/Cooldown rows, and `docs/validation/apple-fitness-interval-parity-dataset/physical-iphone-repeat-block-proof-2026-06-14/` confirms May 20 and June 3 render expanded repeat rows with final `Cooldown` and no `Open / Extra` tail. April 22, 2026 and May 1, 2026 keep the blocked placeholder behavior. Paused repeat blocks, repeat-tail cases, recovery-containing tail cases, missing evidence, and unsupported shapes stay blocked.
...
- Next implementation should keep the narrow normal-detail gates in place and avoid broad interval promotion until paused repeat-block, recovery-containing Open/Extra-tail, and repeat-tail rules are explicitly approved.
