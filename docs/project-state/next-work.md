...
- Paused repeat-block readiness review is documented in `docs/validation/apple-fitness-interval-parity-dataset/paused-repeat-block-promotion-readiness-review-2026-06-15.md`. Next code should add interval timing semantics to `ReconstructedWorkoutInterval`/normal detail first, so elapsed row windows and active/timer duration can both be represented before paused repeat-block promotion.
