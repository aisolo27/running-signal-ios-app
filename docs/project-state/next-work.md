...
- Debug-only Parity Lab Swift scorer now exists in Raw HealthKit Debug/parity packet exports as `customWorkoutCandidateRuleSummary` and `customWorkoutCandidateRuleRows`. Next, regenerate selected-workout parity packets for May 1 and representative repeat/Open-tail cases, then compare the exported active duration, elapsed duration, pause overlap, and Open/Extra tail rows against the docs/debug scorecard. Do not replace normal workout detail interval UI.
