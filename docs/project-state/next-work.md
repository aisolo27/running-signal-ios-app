...
- Ambiguous repeat-tail docs/debug separator rule is now defined in `docs/validation/apple-fitness-interval-parity-dataset/ambiguous-repeat-tail-rule-2026-06-15.md`; Raw HealthKit Debug and parity packet structured comparison now have a debug-only repeat-tail prototype path for no-pause repeat rows with a fixed final planned row and positive residual tail. Next proof is a fresh physical-iPhone export for a repeat-tail case such as June 10 confirming `supported`, expanded repeat rows, fixed final `Cooldown`, and inferred post-final-row `Open / Extra`. Normal-detail UI promotion remains blocked.
