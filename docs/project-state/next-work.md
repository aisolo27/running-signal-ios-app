...
- Next immediate step: spot-check the existing physical-iPhone Parity Lab structured status/fallback output. If the narrow `Warmup(2 km) > one Work step > Cooldown(Open)` class reports cleanly as supported and blocked subclasses report exact fallback reasons, proceed to an internal-gated normal workout detail integration ticket for that narrow class only.
