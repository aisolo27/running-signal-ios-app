...
- Gate B: structured interval workouts remain broadly blocked; row-level FIT extraction exists, and only the clean no-pause `Warmup(2 km) > repeated Work/Recovery rows > Cooldown(Open)` repeat-block class is internally gated into normal detail. Paused repeat blocks, repeat-tail cases, and recovery-containing Open/Extra tail cases are not production-approved.
...
- Internal-gated normal workout detail integration now exists for only the supported no-tail warmup/work/open-cooldown class, the clean fixed-cooldown Open/Extra tail class where planned rows map to HealthKit activities and the only extra row is inferred from the last activity end to workout end, and clean no-pause repeat blocks shaped as `Warmup(2 km) > repeated Work/Recovery rows > Cooldown(Open)`. Physical iPhone screenshots confirmed March 5, 2026 and April 24, 2026 show three Warmup/Work/Cooldown rows, and `docs/validation/apple-fitness-interval-parity-dataset/physical-iphone-repeat-block-proof-2026-06-14/` confirms May 20 and June 3 render expanded repeat rows with final `Cooldown` and no `Open / Extra` tail. April 22, 2026 and May 1, 2026 still show the blocked placeholder. Paused repeat blocks and repeat-tail cases remain blocked.
...
- No broad Gate B subclass is approved for production. Three narrow normal-detail internal gates exist, but paused repeat blocks, recovery-containing tails, repeat-tail cases, simple Work/Open promotion, and broad custom-workout promotion remain blocked.
- Broad normal workout detail interval UI promotion remains blocked. Only the narrow `Warmup(2 km) > one Work step > Cooldown(Open)` class, clean `Warmup(2 km) > one Work step > fixed Cooldown > inferred Open / Extra tail` class, and clean no-pause `Warmup(2 km) > repeated Work/Recovery rows > Cooldown(Open)` repeat-block class are internally gated into normal detail.
