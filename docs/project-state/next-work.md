...
- Debug-only Parity Lab Swift scorer now exists in Raw HealthKit Debug/parity packet exports as `customWorkoutCandidateRuleSummary` and `customWorkoutCandidateRuleRows`. Latest-debug-build iPhone exports for May 1 and representative repeat/Open-tail cases matched the docs/debug scorecard; see `docs/validation/apple-fitness-interval-parity-dataset/fresh-iphone-parity-lab-export-review-2026-06-13.md`. Next, decide whether more balanced evidence is needed before any explicit debug-only Phase 3 prototype discussion. Do not replace normal workout detail interval UI.
