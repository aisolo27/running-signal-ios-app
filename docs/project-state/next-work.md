...
- Current promotion-readiness decision is `docs/validation/apple-fitness-interval-parity-dataset/custom-workout-promotion-readiness-review-2026-06-15.md`; it recommends Gate A simple fixed-distance `Work > Open / Extra` as the safest next narrow normal-detail promotion candidate.
...
- Gate A: simple fixed-distance Work + Open now has a narrow debug-only `HKWorkoutActivity` prototype in Raw HealthKit Debug and parity packet exports. It must stay out of normal workout detail until a later production promotion task explicitly approves it.
...
- Promotion readiness review is complete in `docs/validation/apple-fitness-interval-parity-dataset/custom-workout-promotion-readiness-review-2026-06-15.md`. Next code-changing work, if explicitly approved, should promote only the exact Gate A simple fixed-distance `Work > Open / Extra` class into normal workout detail with guard tests and physical-iPhone proof. Do not include paused repeat blocks, recovery-containing tails, broad repeat-tail behavior, or analytics.
- Simple fixed-distance Work/Open prototype boundary is now defined in `docs/validation/apple-fitness-interval-parity-dataset/simple-work-open-prototype-decision-2026-06-15.md` and implemented in the debug-only Parity Lab/export path. The next step is physical-iPhone evidence capture for that debug path, not normal-detail UI promotion.
...
- No broad Gate B subclass is approved for production. Five narrow normal-detail internal gates exist, but paused repeat blocks, recovery-containing tails, ambiguous repeat-tail cases, simple Work/Open promotion, and broad custom-workout promotion remain blocked.
- Broad normal workout detail interval UI promotion remains blocked. Only the stopped-early single fixed-distance Work class, narrow `Warmup(2 km) > one Work step > Cooldown(Open)` class, clean `Warmup(2 km) > one Work step > fixed Cooldown > inferred Open / Extra tail` class, clean no-pause `Warmup(2 km) > repeated Work/Recovery rows > Cooldown(Open)` repeat-block class, and clean no-pause `Warmup(2 km) > repeated Work/Recovery rows > fixed Cooldown > inferred Open / Extra tail` class are internally gated into normal detail.
