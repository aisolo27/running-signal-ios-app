...
- Later analytics can reuse reconstructed interval windows to analyze per-row HR, power, cadence, and pace streams. Keep that as a future analysis feature after interval correctness is stable; do not expand the current parity slice into coaching or categorization yet.
...
- No broad Gate B subclass is approved for production. Four narrow normal-detail internal gates exist, but paused repeat blocks, recovery-containing tails, ambiguous repeat-tail cases, simple Work/Open promotion, and broad custom-workout promotion remain blocked.
- Broad normal workout detail interval UI promotion remains blocked. Only the narrow `Warmup(2 km) > one Work step > Cooldown(Open)` class, clean `Warmup(2 km) > one Work step > fixed Cooldown > inferred Open / Extra tail` class, clean no-pause `Warmup(2 km) > repeated Work/Recovery rows > Cooldown(Open)` repeat-block class, and clean no-pause `Warmup(2 km) > repeated Work/Recovery rows > fixed Cooldown > inferred Open / Extra tail` class are internally gated into normal detail.
