...
- Gate B: structured interval workouts remain blocked; row-level FIT extraction exists, but repeat-block expansion, repeat-tail cases, and recovery-containing Open/Extra tail cases are not production-approved.
...
- No broad Gate B subclass is approved for production. Two narrow normal-detail internal gates exist, but repeat blocks, recovery-containing tails, repeat-tail cases, simple Work/Open promotion, and broad custom-workout promotion remain blocked.
- Broad normal workout detail interval UI promotion remains blocked. Only the narrow `Warmup(2 km) > one Work step > Cooldown(Open)` class and clean `Warmup(2 km) > one Work step > fixed Cooldown > inferred Open / Extra tail` class are internally gated into normal detail.
