...
- June 25 user-supplied `Thursday Interval 5km` evidence is archived in `docs/archive/old-validation/user-supplied-repeat-tail-review-2026-06-25/`. The current-build re-export passes the fresh readable-label validator and the FIT file confirms an offline session-minus-laps tail residual, but it reports `pairedPauseCount == 0`, so it is not rung 2 paired-pause evidence.
