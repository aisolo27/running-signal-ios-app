...
- Verify goal-distance-clipped fixed-distance Work/Recovery rows against real structured workouts on the physical iPhone, especially HR and cadence values in the interval detail UI.
...
- Workout Plan now leads with a stacked runner-facing prescription summary: warm-up, repeated work target, indented recovery, and cool-down, using compact uniform typography while the expanded raw WorkoutKit plan rows sit behind a disclosure.
