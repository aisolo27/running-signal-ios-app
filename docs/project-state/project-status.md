...
- Heart-rate zones support Automatic HRR, percentage of maximum HR, and manual limits. Automatic HRR uses the latest Apple Health resting HR plus a six-month completed-running maximum and supports a confirmed maximum override. Manual mode uses tappable Zone 1–5 rows with Apple-style lower/upper editors and automatically contiguous adjacent limits. Saving clearly distinguishes previews from the active effective-dated profile; a warned reset can deliberately replace history with the current profile. Workout heart-rate detail shows zone-colored samples and estimated pause-aware time in Zones 1–5.
