...
- Best Efforts displays exact and exact-total records only. A dedicated resumable distance-only history pass checks every eligible run without expanding the bounded full-evidence queue; the screen shows checked/pending coverage under `Verified Best Efforts` until verification is complete, then promotes the heading to `All-Time Records`. Estimates and a provisional-estimates section are intentionally absent.
