...
`hkworkoutactivity-boundary-investigation.md` reviews the regenerated physical-device activity pass. `hkworkoutactivity-boundary-scorecard.md` scores the activity-boundary candidate against the active fixture set. Current read: `HKWorkoutActivity` improves May 26, June 1, and June 12 drift rows, but June 4 regresses from preferred pass to temporary pass and June 3 has three special-fixture regressions. This does not approve production behavior.
...
Raw HealthKit Debug also includes a diagnostics-only monthly refresh/export workflow for a selected month. From any workout in Raw HealthKit Debug, select the month, run `Refresh Month Evidence`, then export `Monthly Diagnostics JSON` for a month-level bundle and `Monthly Diagnostics Summary` for a compact markdown index. The refresh invalidates only each workout's cached evidence for that selected month, reruns HealthKit sample/event/WorkoutKit plan/activity queries, records per-workout refresh status, and continues after individual failures. The JSON has `exportVersion`, `selectedMonth`, `generatedAt`, `workoutCount`, a `summary` block, debug-only safety flags, and one record per loaded running workout with refresh status, evidence source, evidence counts, classification, activity-boundary candidate fields, and embedded parity-packet-style fields. This is for offline comparison against HealthFit FIT files and Apple Fitness/manual references only.
...
- Month-scale collection target: on the physical iPhone, refresh month evidence and export February 2026, March 2026, April 2026, May 2026, and June 2026 monthly diagnostics bundles before selecting additional fixture candidates.
...
- Monthly diagnostics export fields: one debug/research-only JSON bundle per selected month, plus optional markdown summary. Use `Refresh Month Evidence` first when building a fresh month-scale dataset. Use the exported refresh status and evidence source fields to find workouts with fresh evidence, remaining missing evidence after refresh, WorkoutKit plans, HKWorkoutActivity rows, Open / Extra tails, no plan, structured intervals, warmup/work/cooldown special cases, and duplicate/same-day extra run candidates.
...
- `hkworkoutactivity-boundary-scorecard.md`: docs/debug scorecard for the `hkworkoutactivity_boundary` candidate against the active fixture set.
