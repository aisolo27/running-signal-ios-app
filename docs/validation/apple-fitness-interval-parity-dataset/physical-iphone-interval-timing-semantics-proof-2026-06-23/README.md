...
| 2026-04-22 post-install | `raw-exports/2026-04-22-post-install-raw-healthkit-debug.txt` | `raw-exports/2026-04-22-post-install-parity-packet.txt` |
...
## Post-Install Export Check

The 2026-04-22 post-install exports were generated by the installed app at `2026-06-23T11:29:34Z` for workout `0335DE40-44C5-48AC-9A05-F32314713DE1`. The parity packet confirms all 12 `reconstructedIntervals` now carry `elapsedDurationSeconds`, `displayDurationSeconds`, and `durationDisplayRule`. For this paused repeat-block case, pause-overlap and active/timer evidence remains in debug-only `customWorkoutCandidateRuleRows`: Recovery 3 preserves `219.7 s` elapsed, `99.8 s` pause overlap, and `119.9 s` active; Recovery 4 preserves `198.1 s` elapsed, `78.3 s` pause overlap, and `119.8 s` active.

The post-install packet keeps `customWorkoutComparisonSummary.status` as `supported`, `fallbackReasons` empty, `normalWorkoutUIChanged` false, `productionIntervalBehaviorChanged` false, and `usesFITRuntimeTruth` false. `customWorkoutCandidateRuleSummary` remains debug/export-only and not shown in normal workout UI, so this does not promote paused repeat blocks.
