...
- Symptom: broad context values such as VO2 Max or resting heart rate stay nil even though the Data UI references broad HealthKit context. Cause: the permission catalog intentionally skips those read types while `HealthKitService.queryHealthContext()` still queries them. Fix: align the permission catalog, query, and UI copy before treating those context metrics as supported.
...
- Rule: `DerivedAnalyticsEngine.intervalCandidates` is a raw HealthKit event-marker candidate path, not the pause-adjusted custom-workout reconstruction path. Do not use it for Tier 3 interval analytics or trusted active-duration row metrics unless it is explicitly rewired through approved reconstruction and covered by tests.
