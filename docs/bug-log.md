...
- Rule: Gate A simple Work/Open support is currently debug-only in Raw HealthKit Debug and parity packet exports. Do not wire it into normal workout detail, interval analytics, or production promotion without a separate explicit approval.
- Rule: real WorkoutKit one-step blocks can export a single Work row with `repeatBlockIndex: 1` and `repeatIndex: 1` (`Block 1: 1x, 1 step(s)`). Treat that as simple Work/Open eligible when all other Gate A rules pass; only `repeatIndex > 1` is a real repeat-block blocker for this debug gate.
