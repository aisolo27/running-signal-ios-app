...
Verification notes:
- Archived linked doc: `docs/archive/completed-plans/09-healthkit-physical-verification.md`
...
- Step 6 completed on June 6, 2026 from the on-phone diagnostics, HealthKit audit, and physical verification exports. Representative-run proof is archived in `docs/archive/completed-plans/09-healthkit-physical-verification.md`.
...
- HealthKit correctness-lock foundation started on June 10, 2026. Added a documented read-only `HealthKitPermissionCatalog`, `NormalizedRun` provenance model, elapsed-time and total-calorie slots, Apple Watch/richest-source duplicate preference, in-app permission review, Apple Fitness golden-validation screen, generated JSON fixture/checklist exports, and validation docs under `docs/validation/`. This does not complete custom workout structure support; that remains blocked until Apple Fitness parity values are manually filled and verified on the physical iPhone.
- Physical-device build verification on June 10, 2026: XcodeBuildMCP detected `AIS17PM` connected on iOS `26.5.1`; `build_run_device` succeeded for `com.adrielsolorzano.runninganalysis` and launched process `23319`. This proves install/launch on the physical iPhone, but Apple Fitness parity still needs on-phone HealthKit loading plus manual fixture values from Apple Fitness.
- WorkoutKit reconstructed-interval boundary refinement started on June 11, 2026. Distance-goal rows now prefer the crossing HealthKit distance sample end only inside a documented small overshoot tolerance, otherwise they fall back to interpolation; package tests cover the Jun 10 Apple Fitness fixture, cursor advancement, tail shrinkage, and keeping segment markers raw/debug-only.
- Physical-iPhone screenshots on June 11, 2026 validated the WorkoutKit + HealthKit reconstructed interval approach for the Jun 10 Apple Watch custom workout within tolerance: warmup 12:26, work rows close, recoveries exactly 1:45, cooldown 15:12, Open / Extra 0:08, and segment markers still raw/debug-only.
- June 12, 2026 documentation-informed interval parity re-check: Apple documentation defines the public WorkoutKit/HealthKit model, not the private Apple Fitness row-rendering algorithm. The evidence contract stays unchanged: WorkoutKit plan for structure, HealthKit samples for stats, Apple Fitness screenshots for visual parity, Raw HealthKit Debug exports for RunSignal evidence, and HealthKit Segment Markers raw/debug-only. Do not merge post-completed-cooldown activity into `Cooldown` by default.
- June 12, 2026 targeted interval refinement: final planned open cooldown steps now keep the `Cooldown` label through workout end, resolving the June 3 label/structure blocker as a temporary pass. Fixed distance/time cooldowns still create `Open / Extra` when continued running remains. Raw HealthKit Debug now exports compact distance-boundary neighbor samples and final-tail diagnostics for re-exporting June 1 before any boundary-rule change.
